...
def main(config: DictConfig):
    ...
    seq_len = config_model.context_length
    ...
    for name in test_names:
        ...
        if name == "utsd":
            ...
            gift_eval = GiftEval(
                path="data/", input_len=seq_len, output_len=eval_target_len, stride=16
            )
            ...
            artificial = SyntheticTimeSeriesDataset(
                seq_len=seq_len, target_len=eval_target_len, noise=True, n_samples=1000
            )
    ...
    revin_name = config_model.normalizer_name
    use_asinh = config_model.use_asinh

    kv_cache_if_possible = settings.kv_cache_if_possible

    print(
        f"Running experiment with {revin_name}, use_asinh={use_asinh}, kv_cache_if_possible={kv_cache_if_possible}."
    )
